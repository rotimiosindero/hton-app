(function(){
  ...
  // A favourite is keyed by day__stage__begin__name, so publishing a corrected
  // lineup silently wipes the star off every set whose start time or name
  // changed - 24 of them in the July 2026 timetable update alone. Rather than
  // let that happen, re-point orphaned keys at the set they clearly became.
  //
  // Matching is only accepted when it is unambiguous (exactly one candidate),
  // trying in order of confidence: same slot but retimed, then same time but
  // renamed, then same act moved to another stage. Anything still unmatched is
  // deliberately LEFT in storage rather than deleted, so a later data fix can
  // still bring it back instead of the star being gone for good.
  function migrateFavorites(){
    const stored = Object.keys(favorites);
    if(!stored.length) return 0;
    const valid = new Set(entries.map(entryKey));
    const byStageName = new Map(), byStageTime = new Map(), byDayName = new Map();
    const add = (m, k, e) => { if(!m.has(k)) m.set(k, []); m.get(k).push(e); };
    for(const e of entries){
      add(byStageName, `${e.day}__${e.stage}__${e.name}`, e);
      add(byStageTime, `${e.day}__${e.stage}__${e.begin}`, e);
      add(byDayName, `${e.day}__${e.name}`, e);
    }
    let moved = 0;
    for(const key of stored){
      ...
      const [day, stage, begin] = parts;
      const name = parts.slice(3).join('__');   // names never contain "__"
      const attempts = [
        byStageName.get(`${day}__${stage}__${name}`),   // retimed
        byStageTime.get(`${day}__${stage}__${begin}`),  // renamed
        byDayName.get(`${day}__${name}`)                // moved stage
      ];
      for(const cands of attempts){
        if(cands && cands.length === 1){
          const fresh = entryKey(cands[0]);
          if(!valid.has(fresh)) break;          // candidate itself is stale
          if(!(fresh in favorites)) favorites[fresh] = favorites[key];
  ...
  // Content comes from whatsnew.js, which is generated by diffing the shipped
  // data against the previous version. If it has nothing to report the About
  // page link stays hidden, so this cannot go stale on its own.
  const whatsNewOverlay = document.getElementById('whatsNewOverlay');
  const whatsNewRow = document.getElementById('whatsNewRow');
  function buildWhatsNew(){
    const data = (typeof WHATS_NEW !== 'undefined' && WHATS_NEW) ? WHATS_NEW : null;
    const groups = data ? [
      ['Time changes', data.retimed || []],
      ['New acts', data.added || []],
      ['Acts no longer listed', data.removed || []]
    ] : [];
    const total = groups.reduce((n, g) => n + g[1].length, 0);
    const [hy, hm, hd, hh, hmi] = data && data.hideAfter ? data.hideAfter : [];
    const expired = hy !== undefined && new Date() > new Date(hy, hm, hd, hh, hmi);
    if(!total || expired){
      whatsNewRow.style.display = 'none';
      return;
    }
    whatsNewRow.style.display = '';
    document.getElementById('whatsNewTitle').textContent = data.title || 'New Timetable';
    document.getElementById('whatsNewNote').textContent = data.note || '';
    const body = document.getElementById('whatsNewBody');
    body.innerHTML = groups.filter(g => g[1].length).map(([label, rows]) => {
      const items = rows.map(r => {
        const when = r.was
          ? `<span class="whatsnew-was">${escapeHtml(r.was)}</span> &rarr; ` +
            `<span class="whatsnew-now">${escapeHtml(r.now)}</span>`
          : `<span class="whatsnew-now">${escapeHtml(r.time || '')}</span>`;
        return `<div class="whatsnew-item">
            <div class="whatsnew-item-name">${escapeHtml(r.name)}</div>
            <div class="whatsnew-item-meta">
              <span class="whatsnew-day">${escapeHtml(r.day)}</span>
              &nbsp;&middot;&nbsp;${escapeHtml(r.stage)}&nbsp;&middot;&nbsp;${when}
            </div>
          </div>`;
      }).join('');
      return `<p class="whatsnew-section">${escapeHtml(label)} (${rows.length})</p>${items}`;
    }).join('');
  }
  whatsNewRow.addEventListener('click', ()=> whatsNewOverlay.classList.add('open'));
  ...
  loadFavorites();
  migrateFavorites();   // re-point stars whose set was retimed/renamed by a lineup update
  build();
  buildMyHtonGrid();
  renderMyHtonList();
  renderNowLine();
